...
async function main(): Promise<void> {
  ...
  // Generate markdown — OKF concept frontmatter first (SPEC-DOCS-OKF-001/RF-2/RF-3):
  // this file is regenerated on every run, so the generator must emit it
  ...
  const lines: string[] = [
    ...
    '# Traceability Matrix',
    '',
    `> Generated by \`scripts/trace.ts\` on ${today}`,
    '',
  ];
