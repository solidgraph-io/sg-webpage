...
- **RF-6 (progressive enhancement)** — un snippet inline en `<head>` añade `documentElement.classList.add('js')`; el estado inicial oculto de `[data-reveal]` **solo** aplica con `.js` (sin JS → contenido visible). *(Ajuste sobre el template para no ocultar contenido sin JS.)*
