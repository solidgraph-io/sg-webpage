...
- **RF-7 (env en runtime, `astro:env`)** — toda la config de env (`LEAD_*`, `RESEND_API_KEY`, `TURNSTILE_*`) se lee vía **`astro:env`** (`context: 'server'`), **no** `process.env` ni `import.meta.env` directos. Motivo: el contenedor recibe las vars de **Dokploy en runtime** → hay que leerlas en runtime. **Todas** las vars —incluida `TURNSTILE_SITE_KEY`— deben declararse **`access: 'secret'`**: en `astro:env`, las `server + public` se **inlinean en build** (donde el env no existe → valor vacío en el contenedor), mientras que las `secret` se **leen en runtime**. Que la site key sea `secret` a nivel de `astro:env` no la oculta del HTML: sigue emitiéndose como `data-sitekey` (su exposición pública natural). Esto arregla el `500` en `pnpm dev` y, además, el **widget de Turnstile que no cargaba en prod** porque `TURNSTILE_SITE_KEY` (declarada `public`) se horneaba vacía en build. (Fix env: prompt 23; fix site key `public`→`secret`: prompt 45.)
