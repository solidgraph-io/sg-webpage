...
| EPIC-50 | **Actualización de dependencias** (drift ~1 major): Zod 3→4, Astro 5→7 (+@astrojs/node →11, Vite 8/Rolldown), TS 5→6, ESLint 9→10, Vitest 3→4. Cada salto en su rama detrás del gate de fidelidad + tests. Verificar versiones en vivo (`pnpm view … dist-tags`) antes. **OJO:** el compilador Rust de Astro 7 colapsa whitespace (JSX) → puede mover el render fiel; el gate QA-001 lo cubre. | Diferido |
